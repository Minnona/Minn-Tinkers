...
local module = {
    name = "Envenomed Weapons buff",
    desc = "Shows a one-click party poison buff button when Envenomed Weapons is missing or about to expire in a dungeon.",
    category = "Venomancer",
    characterRule = {
        class = "Venomancer"
    },
...
function module:ResolveSpell(core)
    ...
    local spellName = db.spellName or "Envenomed Weapons"
...
function module:IsSpellAvailable(core)
...
function module:CreateButton(core)
    ...
    button.minnTooltipText = "Click to cast Envenomed Weapons when the poison buff is missing or close to expiring."

    if core.SkinSpellButton then
        core:SkinSpellButton(button, "Envenomed Weapons")
    end

    button:SetScript("PostClick", function()
        module.nextImmediateCheck = 0.6
    end)
...
function module:ShowBuffButton(core, manual)
    ...
    if not self:IsSpellAvailable(core) then
        self:HideButton()
        if manual then
            core:Print(tostring(spellName or "Configured poison buff") .. " does not look known/usable on this character.")
        end
        return
    end
    ...
    if not needsBuff then
        ...
        if manual then
            if remaining then
                core:Print(tostring(spellName or "Configured poison buff") .. " is active for another " .. FormatTime(remaining) .. ".")
            else
                core:Print(tostring(spellName or "Configured poison buff") .. " is already active.")
            end
        end
    ...
    if not spellName or spellName == "" then
        if manual then
            core:Print("Could not find configured Envenomed Weapons spell name.")
        end
        return
    end
    ...
    if not self:ConfigureButton(core) then
        if manual then
            core:Print("Cannot prepare Envenomed Weapons button while in combat. Try again after combat.")
        end
        return
    end
    ...
    if manual or (db.printReminder and self.printedForInstanceKey ~= instanceKey) then
        if remaining then
            core:Print("Click the Envenomed Weapons button to refresh " .. tostring(spellName) .. ". Remaining: " .. FormatTime(remaining) .. ".")
        else
            core:Print("Click the Envenomed Weapons button to cast " .. tostring(spellName) .. ".")
        end
...
function module:BuildOptions(core, panel, y)
    ...
    local printReminder = core:CreateCheckbox(
        panel,
        "MinnTinkers_EnvenomedWeapons_PrintReminder",
        "Print Envenomed Weapons reminder",
        "Print Envenomed Weapons reminder",
        "Shows one chat reminder when the one-click Envenomed Weapons button appears.",
        42,
        y,
    ...
    local useRaids = core:CreateCheckbox(
        panel,
        "MinnTinkers_EnvenomedWeapons_UseRaids",
        "Also offer Envenomed Weapons in raids",
        "Also offer Envenomed Weapons in raids",
...
MT:RegisterModule("EnvenomedWeapons", module)
